...
class DatasetUSRNet(data.Dataset):
    ...
    # -----------------------------------------
    '''
    def __init__(self, data_path, sf, sigma, n_channels, batch_size, mode):
        super(DatasetUSRNet, self).__init__()
        self.n_channels = n_channels
        self.patch_size = 96
        self.sigma_max = sigma
        self.scales = [1,2,3,4]
        self.sf_validation = sf
        self.mode = mode
        self.batch_size = batch_size
        #self.kernels = hdf5storage.loadmat(os.path.join('kernels', 'kernels_12.mat'))['kernels']
        self.kernels = loadmat(os.path.join('kernels_12.mat'))['kernels']  # for validation

        # -------------------
        # get the path of H
        # -------------------
        self.paths_H = util.get_image_paths(data_path)  # return None if input is None
        self.count = 0
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            k = self.kernels[0, 2].astype(np.float64)  # validation kernel
            k /= np.sum(k)
