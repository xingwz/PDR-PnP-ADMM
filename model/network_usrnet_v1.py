...
# class Relax_DataNet(nn.Module):
#     def __init__(self):
#         super(Relax_DataNet, self).__init__()
...
class Allin1_HyPaNet(nn.Module):
    def __init__(self, in_nc=2, out_nc=10, channel=64):
        ...
        self.clamp = nn.Sequential(
                ...
                nn.Conv2d(channel, channel, 1, padding=0, bias=True),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel, out_nc//2, 1, padding=0, bias=True),
                nn.Sigmoid())
...
class USRNet(nn.Module):
    def __init__(self, n_iter=8, h_nc=64, in_nc=4, out_nc=3, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        ...
        self.h = HyPaNet(in_nc=2, out_nc=n_iter*2, channel=h_nc) # HyPaNet
        self.n = n_iter
    ...
    def forward(self, x, k, sf, sigma):
        ...
        z = nn.functional.interpolate(x, scale_factor=sf, mode='nearest')
        ...
        for i in range(self.n):
            ...
            # alpha = ab[:, i+self.n*2:i+self.n*2+1, ...]
            # x = (1-alpha)*z + alpha*x
            z = self.p(torch.cat((x, ab[:, i+self.n:i+self.n+1, ...].repeat(1, 1, x.size(2), x.size(3))), dim=1))
...
# --------------------------------------------
# main URSRNet
# deep unfolding relaxed super-resolution network
# --------------------------------------------
"""
    
class URADMMNet(nn.Module):
    def __init__(self, n_iter=8, h_nc=64, in_nc=4, out_nc=3, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        super(URADMMNet, self).__init__()

        self.d = DataNet()
        self.p = ResUNet(in_nc=in_nc, out_nc=out_nc, nc=nc, nb=nb, act_mode=act_mode, downsample_mode=downsample_mode, upsample_mode=upsample_mode)
        self.h = Allin1_HyPaNet(in_nc=2, out_nc=n_iter*2, channel=h_nc) # Allin1_HyPaNet
        self.n = n_iter
    ...
    def forward(self, x, k, sf, sigma):
        ...
        u = z*0
         ...
         # hyper-parameter, alpha & beta
        params = self.h(torch.cat((sigma, torch.tensor(sf).type_as(sigma).expand_as(sigma)), dim=1))

         # unfolding
        for i in range(self.n):
            
            x = self.d((z-u), FB, FBC, F2B, FBFy, params[:, i:i+1, ...], sf)
            alpha = params[:, i+self.n*2:i+self.n*2+1, ...]
            x = (1-alpha)*(z-u) + alpha*x
            z = self.p(torch.cat(((x+u), params[:, i+self.n:i+self.n+1, ...].repeat(1, 1, x.size(2), x.size(3))), dim=1))
            u = u + (x - z)
        return x
