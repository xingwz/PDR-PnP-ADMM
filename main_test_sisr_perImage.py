...
parser = argparse.ArgumentParser(description='SISR PnP_ADMM_Relaxation *^_^*')
...
parser.add_argument('--reuslt_path', type=str, required=False, dest='reuslt_path',
                    default='results/SISR_PnP/R_ADMM_Image/')
parser.add_argument('--n_channels', type=int, required=False, default=1, dest='n_channels')
parser.add_argument('--mu', type=float, required=False, dest='mu', default=7.5522e-05) # from ADMM_Relax_H3
parser.add_argument('--thr1', type=float, required=False, dest='thr1', default=9.1854e-02) # from ADMM_Relax_H3
parser.add_argument('--alpha', type=float, required=False, dest='alpha', default=8.9326e-01) # from ADMM_Relax_H3, fixed
...
#    print(i+1, ',process image:',m[-1])
    ...
    # Hyper-parameters
    mu = torch.FloatTensor([config.mu]).view([1,1,1,1]) #
    thr1 = torch.FloatTensor([config.thr1]).view([1,1,1,1])
    ...
    for j in range(config.iter_num):
        ...
        x = sr.data_solution((z-u).float(), FB, FBC, F2B, FBFy, mu_tensor, config.sf)
        x = (1-config.alpha)*(z-u) + config.alpha*x
        
        # --------------------------------
        # step 2, W (Z) update
        # --------------------------------
        
        x_u_sigma = torch.cat(((x+u).float(), thr1_tensor.repeat(1, 1, interp_L.shape[2], interp_L.shape[3])), dim=1)
